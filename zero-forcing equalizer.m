...
for ii = 1:length(Eb_N0_dB)
    ...
    % grouping in [nRx,nTx,N/nTx] matrix
    
    h = 1/sqrt(2) * (randn(nRx, nTx, N/nTx) + 1j*randn(nRx, nTx, N/nTx)); % Rayleigh channel
    ...
    % Receiver
    % Forming the Zero Forcing equalization matrix W = inv(H'*H)*H'
    hCof = zeros(2,2,N/nTx);
    hCof(1,1,:) = sum(h(:,2,:) .* conj(h(:,2,:)), 1); % a term
    hCof(2,2,:) = sum(h(:,1,:) .* conj(h(:,1,:)), 1); % d term
    ...
    hInv = hCof ./ hDen; % inv(H'*H)
    
    hMod = reshape(conj(h), nRx, N); % H' operation
    yMod = kron(y, ones(1, 2)); %
    yMod = sum(hMod .* yMod, 1); % H'*y
    yMod = kron(reshape(yMod, 2, N/nTx), ones(1, 2)); % formatting
    yHat = sum(reshape(hInv, 2, N) .* yMod, 1); % inv(H'*H)*H'*y
...
title('BER for BPSK modulation with 2x2 MIMO and ZF equalizer (Rayleigh channel)');cd